...
contract VoteEscrowToken is IVoteEscrowToken, ReentrancyGuard, ERC20, WithPausability, WhitelistedTransfer, TokenRecovery, VoteEscrowBooster, VoteEscrowLocker {
  ...
  constructor(IStore store, string memory tokenName, string memory tokenSymbol) ERC20(tokenName, tokenSymbol) {
    s = store;
  }
  ...
  function _unlockWithPenalty(uint256 penalty) internal {
    ...
    // Transfer NPM
    _npm.safeTransfer(msg.sender, amount - penalty);

    if (penalty > 0) {
      _npm.safeTransfer(_feeTo, amount - penalty);
    }
  }
  ...
  function lock(address account, uint256 amount, uint256 durationInWeeks) external override nonReentrant {
    ...
    if (amount > 0) {
      _npm.safeTransferFrom(account, address(this), amount);
