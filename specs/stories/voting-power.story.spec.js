...
describe('Vote Escrow Token: Voting Power Story', () => {
  ...
  it('bob locks 100 NPM at t0 for 208 weeks', async () => {
    const [, bob, charlie] = await ethers.getSigners()
    await contracts.npm.connect(bob).approve(contracts.veToken.address, helper.ether(100))

    await contracts.veToken.connect(bob).lock(helper.ether(100), 208)
    t0 = await time.latest()
    const latestBlockNumber = await time.latestBlock()

    ;(await contracts.veToken._balances(bob.address)).should.equal(helper.ether(100))
  ...
  it('At t0 + 2 weeks, charlie locks 200 NPM for 8 weeks', async () => {
    ...
    await contracts.npm.connect(charlie).approve(contracts.veToken.address, helper.ether(200))

    const week2 = t0 + 2 * WEEKS
    await time.setNextBlockTimestamp(week2)
    await contracts.veToken.connect(charlie).lock(helper.ether(200), 8)

    ;(await contracts.veToken._balances(charlie.address)).should.equal(helper.ether(200))
    ;(await contracts.veToken._unlockAt(charlie.address)).should.equal(week2 + 8 * WEEKS)
  })
  ...
  it('At t0 + 8 week(s), charlie unlocks prematurely and receives 75 NPM', async () => {
    ...
    const tx = await contracts.veToken.connect(bob).unlockPrematurely()
  ...
  it('At t0 + 9 week(s)', async () => {
    ...
    await contracts.veToken.connect(bob).lock(helper.ether(100), 4)
  ...
  it('At t0 + 10 week(s)', async () => {
    ...
    await contracts.veToken.connect(charlie).unlock()
  ...
  it('At t0 + 15 week(s)', async () => {
    ...
    await contracts.veToken.connect(bob).unlock()
