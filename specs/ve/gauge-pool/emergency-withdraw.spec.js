...
describe('Liquidity Gauge Pool: Emergency Withdraw', () => {
  ...
  it('must allow to emergency withdraw', async () => {
    const [owner] = await ethers.getSigners()
    ...
    await contracts.gaugePool.emergencyWithdraw()
    const balanceAfterEmergencyWithdraw = await contracts.fakePod.balanceOf(owner.address)
    balanceAfterEmergencyWithdraw.should.equal(balanceBeforeDeposit)
  })

  it('must not allow to emergency withdraw before lockup period ends', async () => {
    const [owner] = await ethers.getSigners()
    const amountToDeposit = helper.ether(10)
  ...
  it('must allow to emergency withdraw even when paused', async () => {
  ...
  it('throws during reentrancy attack', async () => {
    ...
    const fakePod = await factory.deployUpgradeable('FakeTokenWithReentrancy', key.toBytes32('emergencyWithdraw'))
    ...
    await gaugePool.emergencyWithdraw()
